...
def read(filename='./hospitals.csv'):
...
def write_md(hospital):
    path = 'donate/{}.md'.format(hospital[0])
...
def gerenate_table(hospital):
    table = '|  防护物资（耗材）  |  标准/要求  |\n|------------------|-----------|\n'
    for supply, standard in zip(hospital[9].split('|'), hospital[10].split('|')):
...
def gerenate_contacts(hospital):
    ...
    if hospital[6]:
        contacts += '电话：{}\n\n'.format(hospital[6])
    contacts += '联系人：\n\n'
    for contact, mobile in zip(hospital[7].split('|'), hospital[8].split('|')):
